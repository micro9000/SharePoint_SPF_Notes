...
export default class ConsumerWebPartDemo extends React.Component<
  IConsumerWebPartDemoProps,
  IConsumerWebPartDemoState
> {
  constructor(
    props: IConsumerWebPartDemoProps,
    state: IConsumerWebPartDemoState
  ) {
    super(props);
  ...
  private _getListItems(): Promise<IEmployee[]> {
    const url: string =
      this.props.siteUrl +
      "/_api/web/lists/getbytitle('Employees')/items?$filter=DeptTitleId eq " +
